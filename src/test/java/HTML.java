...
public class HTML {
	...
	public void deveFazerBuscasComHTML() {
		given()
			.log().all()
		.when()
			.get("https://restapi.wcaquino.me/v2/users")//interface HTML, abrir no navegador, retorna 2 registros
		.then()
			.log().all()  //deve vir antes de qualquer validação, pois se der erro, não será logado
			.statusCode(200)
